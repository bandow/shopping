...
jQuery.fn.extend({
	Page:function(type,PageClass){
		...
		if(Config.Page!=1){
			$Page.prepend($(PageSon).attr(Config.PageCustom,parseInt(Config.Page)-1).text(Config.TopText).attr({title:Config.TopTitle,class:'page-l'}));
		}
			
		if(Config.Page!=Config.Pages){
			$Page.append($(PageSon).attr(Config.PageCustom,parseInt(Config.Page)+1).text(Config.DownText).attr({title:Config.DownTitle,class:'page-r'}));
		}
